...
/** @return never */
function failBackfill(string $code, string $message, int $exitCode = 1): void
{
    fwrite(STDERR, "FOUT [{$code}]: {$message}\n");
    exit($exitCode);
}

function envBackfill(string $key, bool $required = true): string
{
    $value = $_ENV[$key] ?? $_SERVER[$key] ?? getenv($key);
    $value = $value === false ? '' : trim((string) $value);
    if ($required && $value === '') failBackfill('MISSING_ENV', "{$key} ontbreekt.", 2);
    return $value;
}

/** @return array{mode: 'dry-run'|'execute'|'help', allowDevelopmentDataset: bool} */
function backfillOptions(array $argv): array
{
    $mode = 'dry-run';
    $modeSeen = false;
    $allowDevelopmentDataset = false;
    foreach (array_slice($argv, 1) as $argument) {
        if ($argument === '--help' || $argument === '-h') return ['mode' => 'help', 'allowDevelopmentDataset' => false];
        if ($argument === '--allow-development-dataset') { $allowDevelopmentDataset = true; continue; }
        if ($argument !== '--dry-run' && $argument !== '--execute') failBackfill('UNKNOWN_OPTION', "Onbekende optie: {$argument}", 2);
        if ($modeSeen) failBackfill('CONFLICTING_OPTIONS', 'Gebruik precies één modusvlag.', 2);
        $mode = $argument === '--execute' ? 'execute' : 'dry-run';
        $modeSeen = true;
    }
    if ($allowDevelopmentDataset && $mode !== 'execute') failBackfill('DEVELOPMENT_FLAG_REQUIRES_EXECUTE', 'Gebruik --allow-development-dataset uitsluitend samen met --execute.', 2);
    return ['mode' => $mode, 'allowDevelopmentDataset' => $allowDevelopmentDataset];
}

function printBackfillHelp(): void
{
    fwrite(STDOUT, "Gebruik:\n  php scripts/backfill-booking-price-snapshots.php --dry-run\n  php scripts/backfill-booking-price-snapshots.php --execute\n  php scripts/backfill-booking-price-snapshots.php --execute --allow-development-dataset\n\nStandaardmodus zonder vlag: dry-run. De ontwikkeloptie werkt alleen met APP_ENV=development en nooit op onderwijsboeking_v2. Alleen DB_* wordt gebruikt; de legacybron wordt niet geopend.\n");
}
...
$options = backfillOptions($argv);
$mode = $options['mode'];
if ($mode === 'help') { printBackfillHelp(); exit(0); }

try {
    Dotenv::createImmutable(dirname(__DIR__))->load();
    $host = envBackfill('DB_HOST');
    $port = envBackfill('DB_PORT');
    $database = envBackfill('DB_NAME');
    $user = envBackfill('DB_USER');
    $password = envBackfill('DB_PASSWORD', false);
    $appEnvironment = envBackfill('APP_ENV');
    fwrite(STDOUT, "Doeldatabaseconfiguratie: host={$host}; port={$port}; database={$database}\n");
    if ($options['allowDevelopmentDataset']) {
        (new BackfillExecutionPolicy())->assertDevelopmentDatasetAllowed($appEnvironment, $database);
        fwrite(STDOUT, "\n*** WAARSCHUWING: productiereconciliatie wordt uitsluitend voor deze lokale ontwikkelrun overgeslagen. ***\n\n");
    }
    ...
    if ($mode === 'dry-run') {
        ...
        printBackfillAnalysis($result, $mode);
        fwrite(STDOUT, "Transactiestatus: GEEN MUTATIES (read-only dry-run)\n");
        exit($service->productionReconciliationMatches($result) ? 0 : 1);
    }
    $result = $options['allowDevelopmentDataset'] ? $service->executeDevelopmentDataset() : $service->execute();
    printBackfillAnalysis($result, $mode);
    fwrite(STDOUT, "Transactiestatus: COMMIT VOLTOOID\n");
} catch (Throwable $exception) {
    failBackfill('BACKFILL_FAILED', $exception->getMessage());
}
