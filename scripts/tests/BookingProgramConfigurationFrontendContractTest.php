...
$assert(str_contains($panel,'attemptedSteps')&&str_contains($panel,'validateStep(step.value)')&&str_contains($panel,'if(!validateStep(step.value))return'),'Minimumvalidatie blokkeert Volgende niet stapgericht.');
$assert(str_contains($panel,'const found=stepIssues(5)')&&str_contains($panel,'step.value=first')&&str_contains($panel,'void save(false)'),'Stap 5 valideert eerdere stappen niet vóór opslag.');
$assert(strpos($panel,'void save(false)')>strpos($panel,'const found=stepIssues(5)'),'API-opslag kan vóór lokale stap-5-validatie plaatsvinden.');
$assert(str_contains($panel,'stepIssues(stepNumber).length===0')&&str_contains($panel,'state:step.value===stepNumber?"current":completed?"completed"'),'Completed-state vereist geen geldige bevestigde stap.');
...
$assert(str_contains($panel,'issueStep')&&str_contains($panel,'Math.min')&&str_contains($panel,'aria-live="assertive"')&&str_contains($panel,'nextTick'),'Issue-stepmapping, foutnavigatie of aria-live ontbreekt.');
