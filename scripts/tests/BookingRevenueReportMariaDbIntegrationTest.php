...
try{
    ...
    $insert->execute([':school'=>'MariaDB optie',':date'=>'2026-07-15']);$optionId=(int)$pdo->lastInsertId();
    $pdo->prepare("UPDATE aanvragen SET status='In optie' WHERE id=:id")->execute([':id'=>$optionId]);
    $insert->execute([':school'=>'MariaDB afgewezen vroeg',':date'=>'2024-01-01']);$rejectedEarly=(int)$pdo->lastInsertId();
    $pdo->prepare("UPDATE aanvragen SET status='Afgewezen' WHERE id=:id")->execute([':id'=>$rejectedEarly]);
    $insert->execute([':school'=>'MariaDB afgewezen laat',':date'=>'2030-01-01']);$rejectedLate=(int)$pdo->lastInsertId();
    $pdo->prepare("UPDATE aanvragen SET status='Afgewezen' WHERE id=:id")->execute([':id'=>$rejectedLate]);
    ...
    $assert($bounds->minDate==='2026-07-15'&&$bounds->maxDate==='2026-08-01','MariaDB-grenzen gebruiken niet optie plus definitief of tellen afgewezen uitschieters mee.');
    $rows=$repository->findRows(new BookingRevenueCriteria('2026-08-01','2026-08-01'));
