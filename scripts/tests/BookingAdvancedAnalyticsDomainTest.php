...
$booking = static fn (string $date, string $school, string $address, string $postcode, int $students = 40): array => ['status'=>'Definitief','visit_date'=>$date,'school'=>$school,'address'=>$address,'postal_code'=>$postcode,'city'=>'Leerdam','country'=>'Nederland','students'=>$students];
$rows = [
...
$result = (new BookingAdvancedAnalyticsCalculator(new DisabledDatesSqlService($pdo), new BookingDaySettingsSqlRepository($pdo)))->calculate($rows, $criteria, $rows);
...
$overResult = (new BookingAdvancedAnalyticsCalculator(new DisabledDatesSqlService($pdo), new BookingDaySettingsSqlRepository($pdo)))->calculate($overRows, $over, $overRows)['capacityByMonth'][0];
