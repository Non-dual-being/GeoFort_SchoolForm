...
$assert(str_contains($types,'| "conflict"')&&preg_match('/\blunchChoice\s*===\s*[\'"]conflict[\'"]/', $panel)===1&&!str_contains($detailService,"LUNCH_CONFLICT ? 'none'"),'Conflict-readstate wordt niet ongewijzigd doorgegeven.');
$assert(str_contains($panel,'CATERING_CONFLICT')&&str_contains($panelWithoutWhitespace,'emit("completed"')&&str_contains($view,'await load(true)'),'Conflictrefresh/editcontext ontbreekt.');
$assert(str_contains($panelWithoutWhitespace,'conflictDetected.value=true')&&str_contains($panelWithoutWhitespace,'conflictRefreshReady.value=true')&&str_contains($panelWithoutWhitespace,':disabled="conflictDetected||!lunchChoice"')&&str_contains($panelWithoutWhitespace,':disabled="submitting||!conflictRefreshReady"')&&str_contains($panel,'Actuele serverwaarden:')&&str_contains($panel,'Actuele gegevens overnemen'),'Conflict toont geen actuele serverwaarden of kan vóór refresh/zonder expliciete herbeoordeling opnieuw worden opgeslagen.');
