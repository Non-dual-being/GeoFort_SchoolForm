...
export function compareAnalyticsValues(left: string | number | null | undefined, right: string | number | null | undefined, type: AnalyticsSortType): number {
...
export function stableSortAnalyticsRows(rows: AnalyticsRow[], state: AnalyticsSortState | null, types: Record<string, AnalyticsSortType>): AnalyticsRow[] {
  ...
  return rows.map((row, index) => ({ row, index })).sort((a, b) => {
    const leftMissing = a.row[state.key] === null || a.row[state.key] === undefined;
    const rightMissing = b.row[state.key] === null || b.row[state.key] === undefined;
