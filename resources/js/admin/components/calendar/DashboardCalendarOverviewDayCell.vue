...
const stateText = computed(() => {
  ...
  if (props.statusFilter === "all") return grouped.value.map((item) => `${item.count} ${item.label}`).join(", ");
...
<template>
  <div class="admin-calendar-overview-day" role="gridcell" :class="[{ 'is-outside': !day.inSelectedMonth, 'is-blocked': Boolean(day.disabled), 'is-past': day.isPast, 'is-today': day.isToday }, accentClasses]" :aria-label="ariaLabel" :title="ariaLabel">
    ...
    <template v-if="day.inSelectedMonth">
      ...
      <template v-else-if="totalBookings">
        <div v-if="statusFilter === 'all'" class="admin-calendar-overview-day__badges">
          <span v-for="item in grouped" :key="item.value" class="admin-status admin-calendar-overview-badge" :class="calendarOverviewStatusClass(item.presentation)"><strong>{{ item.count }}</strong> {{ item.label }}</span>
        </div>
