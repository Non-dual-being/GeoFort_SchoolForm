...
withDefaults(defineProps<{modelValue:string|null;options:Module[];confirmed:boolean;stepNumber:number;totalSteps:number;confirmationError?:string|null;fieldError?:string|null;staleMessage?:string|null}>(),{confirmationError:null,fieldError:null,staleMessage:null});
...
<template>
  <section class="admin-program-step" aria-labelledby="module-step-title">
    <header><p class="admin-program-step__eyebrow">Stap {{ stepNumber }} van {{ totalSteps }}</p><h3 id="module-step-title">Kies een keuzemodule</h3><p>De beschikbare keuzemodules worden aangepast aan uw onderwijsselectie.</p></header>
